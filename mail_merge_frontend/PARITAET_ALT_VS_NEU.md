...
`serienbrief_vorlage.py`. Quelle neu: `mail_merge_frontend/src_react/` + Page
...
| A5 | Editor-Zugriffsmodus nach Schreibrecht | ◑ | nur `can_write` (write-Permission) → read-only. Keine Rollen-Feindifferenzierung. |
...
| # | Feature (alt) | neu | Anmerkung |
|---|---|---|---|
| E1 | PDF-Vorschau (Split-Preview Beispielwerte + echter Zielobjekt) | ✓ | |
| E2 | Zielobjekt-Selektor | ✓ | echte Zielobjekte, Suche. |
| E3 | Iterations-Doctype-Mismatch-Handling beim Zielobjekt | ◑ | neu lädt Zielobjekt nach haupt_verteil_objekt, aber kein expliziter Reset/Mismatch-Hinweis. |
