...
const RECIPIENTS = [
  ...
  { id: "MV-2024-0142", label: "Müller, Andreas", sub: "Tristanstr. 4, WE 03 · Zielobjekt" },
  { id: "MV-2023-0098", label: "Schäfer, Marlene", sub: "Tristanstr. 4, WE 07 · Zielobjekt" },
  { id: "MV-2021-0034", label: "Bauer, Reinhold", sub: "Leopoldstr. 88, WE 12 · Zielobjekt · Statuscode 2" },
];
...
const PreviewPane = ({ tpl, folder, onClose, onOpen, onDurchlauf }) => {
  ...
  // Echte Zielobjekt laden (nur eingebettet), abhängig vom Verteil-Objekt.
  useEffect(() => {
  ...
  // Echtes PDF rendern (nur eingebettet), wenn Vorlage/Zielobjekt wechselt.
  useEffect(() => {
    ...
    <aside className="bw-preview">
      ...
      <div className="bw-preview-recipient">
        <button className="bw-preview-recipient-btn" onClick={() => setRecipientPickerOpen(o => !o)}>
          ...
          <span className="bw-preview-recipient-label">Zielobjekt</span>
      ...
      <div className="bw-preview-pdf">
        {embedded ? (
          ...
          <div className="bw-preview-pdf-paper">
            ...
            <br/>
            <div>{recipient.label === "Beispielwerte" ? "Musterperson" : recipient.label}</div>
...
export const App = () => {
  ...
  const [openKeys, setOpenKeys] = useState(() => new Set(["erinnerungen", "datensatz"]));
  ...
  const handleCreateTemplate = useCallback(() => {
    ...
    if (!title || !title.trim()) return;
    createTemplate(title.trim(), selectedFolder, "Zielobjekt")
