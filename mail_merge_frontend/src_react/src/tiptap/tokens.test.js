...
describe("Token-Round-Trip", () => {
	it("einfacher Platzhalter im Absatz", () => {
		expectTokensPreserved("<p>Sehr geehrte/r {{ person.nachname }},</p>");
	});
	...
	it("Platzhalter in Mark (fett)", () => {
		expectTokensPreserved("<p>Offen: <strong>{{ offener_betrag }}</strong></p>");
	});
	...
	it("Marks um Chips bleiben nach Round-Trip erhalten", () => {
		...
		const bold = roundTrip("<p>Offener Betrag: <strong>{{ offener_betrag }}</strong> EUR</p>");
		expect(bold).toMatch(/<strong>\{\{\s*offener_betrag\s*\}\}<\/strong>/);

		const italic = roundTrip("<p><em>{{ person.nachname }}</em></p>");
		expect(italic).toMatch(/<em>\{\{\s*person\.nachname\s*\}\}<\/em>/);

		const sized = roundTrip(
			'<p>Wert: <span style="font-size: 14pt">{{ offener_betrag }}</span></p>'
		);
		expect(sized).toMatch(/font-size:\s*14pt[^>]*>\{\{\s*offener_betrag\s*\}\}</);
	...
	it("Umlaute und mehrere Platzhalter", () => {
		expectTokensPreserved(
			"<p>Objekt {{ objekt.bezeichnung }} in {{ standort.bezeichnung }} – Größe {{ objekt.qm }} m²</p>"
		);
	});

	it("Baustein mit Quotes und Umlaut bleibt roh", () => {
		const out = expectTokensPreserved('<div>{{ baustein("Fußzeile Zahlungsdaten") }}</div>');
		expect(out).toContain('{{ baustein("Fußzeile Zahlungsdaten") }}');
	});

	it("Block-Level {% if %} zwischen Absätzen", () => {
		const out = expectTokensPreserved(
			"<p>Text vorher.</p>\n{% if first %}\n<p>Bedingter Absatz {{ offener_betrag }}.</p>\n{% endif %}\n<p>Danach.</p>"
		);
	...
	it("legacy data-token Chip round-trippt (Abwärtskompatibilität)", () => {
		const legacy =
			'<p>Hallo <span class="chip" data-group="person" data-token="{{ person.vorname }}" contenteditable="false">person.vorname</span>!</p>';
		...
		expect(tokenMultiset(out)).toEqual({ "{{ person.vorname }}": 1 });
	});
});
...
describe("if-Bedingungen (inline hvIf)", () => {
	...
	it("Vergleich == bleibt erhalten (Feld-Chip + Text)", () => {
		const out = expectTokensPreserved('<p>a</p>\n{% if statuscode == "2" %}\n<p>x</p>\n{% endif %}');
		expect(out).toContain('{% if statuscode == "2" %}');
	});
