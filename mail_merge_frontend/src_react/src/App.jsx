...
// Sentinel-Zielobjekt „Beispielwerte" (kein echter Datensatz → Split-Preview).
...
export const App = () => {
  ...
  const onTemplateSelect = useCallback(async (id) => {
    ...
    if (id === "t-001") {
      ...
      setTemplate({
        ...
        id,
        title: t.title,
        htmlContent: `<h2>${t.title}</h2><p>Diese Vorlage ist im Prototyp nicht hinterlegt. Wechsel zurück zu „1. Erinnerung" für die volle Demo.</p>`,
      });
      setTitle(t.title);
    }
    setDirty(false);
    ...
    const walk = (nodes) => {
      for (const n of nodes || []) {
        if (n.token) {
          ...
          // reiner Pfad „x". Das $ MUSS mit weg, sonst landet „$ objekt.objekt $"
  ...
  // nicht am Zielobjekt oder am Edit-Stand — daher kein Debounce, ein-Fetch reicht.
  useEffect(() => {
  ...
  // Sofort rendern bei Tab-/Vorlagen-/Zielobjektwechsel (Signatur-Check dedupt).
        ...
        <div className="modal-backdrop" onClick={() => setRecipientPickerOpen(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <div>
                <div style={{ fontWeight: 600, fontSize: 14 }}>Zielobjekt für Vorschau wählen</div>
                <div style={{ fontSize: 12, color: "var(--text-muted)" }}>Die Vorlage wird für den gewählten Zielobjekt gerendert — Platzhalter, Bausteine und Bedingungen werden ausgewertet.</div>
              </div>
            ...
            <div className="modal-body">
              ...
              >
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 11, color: "var(--text-faint)" }}>Vorschau mit Musterdaten (kein echter Zielobjekt)</div>
                </div>
