    ...
    <header className="dl-header">
      ...
      <div className="dl-stats">
        <div className="dl-stat">
          <div className="dl-stat-label">Zielobjekt</div>
        ...
        <div className="dl-stat warn">
          ...
          <div className="dl-stat-sub">{stats.skipped > 0 ? "z. B. Offener Betrag = 0" : "—"}</div>
        </div>
...
const RecipientRow = ({ r, selected, onSelect, onToggleSelect, isCurrent, onDownload, hasOverrides, overrideCount }) => {
  ...
  const offener_betragNum = parseFloat(r.offener_betrag.replace(/[.,]/g, m => m === "," ? "." : "").replace("–", "-").replace("€", ""));
  const offener_betragCls = isNaN(offener_betragNum) ? "" : offener_betragNum < 0 ? "negative" : offener_betragNum === 0 ? "zero" : "";
  ...
  return (
    <div className={`dl-row ${isCurrent ? "selected" : ""}`} onClick={() => onSelect(r)}>
      ...
      <div className={`dl-cell dl-cell-offener_betrag ${offener_betragCls}`}>{r.offener_betrag}</div>
  ...
  return (
    <main className="dl-recipients">
      <div className="dl-recipients-head">
        <div className="dl-recipients-title">Zielobjekt</div>
        ...
        <div className="dl-recipients-search">
          ...
          <input placeholder="Zielobjekt suchen…" value={query} onChange={e => onQuery(e.target.value)}/>
        </div>
      ...
      {selectedIds.size > 0 && (
        <div className="dl-bulk-bar">
          <div className="dl-bulk-info">
            ...
            <span>{selectedIds.size === 1 ? "Zielobjekt" : "Zielobjekt"} ausgewählt</span>
          </div>
      ...
      <div className="dl-list">
        <div className="dl-list-header">
          ...
          <div className="dl-cell">Zielobjekt</div>
          <div className="dl-cell" style={{ textAlign: "right", paddingRight: 16 }}>Offener Betrag</div>
        ...
        {recipients.length === 0 ? (
          <div style={{ padding: 40, textAlign: "center", color: "var(--text-muted)" }}>
            Keine Zielobjekt für diesen Filter.
          </div>
        ) : recipients.map(r => (
          <RecipientRow
            key={r.id}
    ...
    return (
      <aside className="dl-detail">
        <div className="dl-detail-empty">
          ...
          <div className="dl-detail-empty-title">Zielobjekt-Details</div>
          <div className="dl-detail-empty-sub">Klicke links auf einen Zielobjekt, um PDF, Variablen und Render-Log zu sehen.</div>
        </div>
      </aside>
    );
  }
      ...
      <div className="dl-detail-body">
          ...
          <div className="dl-pdf-stage">
            {r.status === "skipped" ? (
              ...
              <div className="dl-pdf-paper">
                ...
                <br/>
                <p>auf dem Konto Ihrer Objekt in der {r.address.split("·")[0].trim()} besteht aktuell ein offener Offener Betrag in Höhe von <strong>{r.offener_betrag}</strong>.</p>
                <br/>
                ...
                {r.statuscode === 2 && (
                  <>
                    <br/>
                    <p>Da es sich um die zweite Erinnerung handelt, erheben wir eine Bearbeitungsgebuehr in Höhe von {durchlauf.variables.find(v => v.name === "gebuehr")?.value} €.</p>
                  </>
                )}
                <br/>
                <p>Sollten Sie den Betrag bereits überwiesen haben, betrachten Sie dieses Schreiben bitte als gegenstandslos.</p>
                <br/>
                <p>Mit freundlichen Grüßen<br/>Beispiel GmbH</p>
              </div>
            )}
          </div>
        )}
        ...
        {tab === "log" && (
          <div className="dl-log-list">
            ...
            <div className="dl-log-line"><span className="dl-log-time">09:16:21.418</span><span className="dl-log-level dl-log-level-info">INFO</span><span>Resolving baustein paths for haupt_verteil_objekt=Zielobjekt</span></div>
            <div className="dl-log-line"><span className="dl-log-time">09:16:21.422</span><span className="dl-log-level dl-log-level-ok">OK</span><span>Anrede formal → person=objekt.customer</span></div>
            <div className="dl-log-line"><span className="dl-log-time">09:16:21.425</span><span className="dl-log-level dl-log-level-ok">OK</span><span>Offener Betrag-Berechnung → datensatz=objekt</span></div>
            <div className="dl-log-line"><span className="dl-log-time">09:16:21.427</span><span className="dl-log-level dl-log-level-ok">OK</span><span>Output: offener_betrag={r.offener_betrag}, statuscode={r.statuscode}</span></div>
            {r.statuscode === 2 && <div className="dl-log-line"><span className="dl-log-time">09:16:21.430</span><span className="dl-log-level dl-log-level-info">INFO</span><span>Jinja-if statuscode == "2" evaluated TRUE</span></div>}
  ...
  return (
    <div className="durchlauf-app dl-new">
      <div className="dl-new-card">
        ...
        <input className="dl-new-input" value={title} placeholder="z. B. Serienlauf Mai 2026" onChange={(e) => setTitle(e.target.value)}/>
