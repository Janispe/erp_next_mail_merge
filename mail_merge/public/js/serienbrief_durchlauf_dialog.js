...
const hv_sbd_open_vorlage_picker = (dialog) => {
	const picker = mail_merge.serienbrief.open_vorlage_picker;
...
const hv_sbd_open_iteration_picker = (dialog, state) => {
	...
	// "Make {Doctype}"-Secondary-Button aus dem
	// MultiSelectDialog-Default verstecken — im Serienbrief-Flow soll der User
	// kein neues Ziel-Dokument anlegen, nur auswählen. Frappe hardcoded
	...
	if (window.mail_merge?.ui?.widen_modal) window.mail_merge.ui.widen_modal(inner);
};
