...
//
// API:
//   mail_merge.serienbrief.mount_vorlagen_browser($container, opts)
//     -> { refresh, set_kategorie, destroy }
//   mail_merge.serienbrief.open_vorlage_picker({ title, on_pick, initial_kategorie })
//
// opts.mode: "manage" (Default) = alle CRUD-Aktionen; "picker" = nur Auswahl.

frappe.provide("mail_merge.serienbrief");
...
const MAIL_MERGE_BROWSER_STYLE_ID = "mail-merge-vorlagen-browser-styles";

const hv_browser_ensure_styles = () => {
	if (document.getElementById(MAIL_MERGE_BROWSER_STYLE_ID)) return;
	const style = document.createElement("style");
	style.id = MAIL_MERGE_BROWSER_STYLE_ID;
	style.textContent = `
...
const hv_browser_open_fulltext_dialog = (on_pick) => {
	const dialog = new frappe.ui.Dialog({
		...
		primary_action(values) {
			...
			frappe
				.call({
					method: "mail_merge.mail_merge.doctype.serienbrief_vorlage.serienbrief_vorlage.search_serienbrief_vorlagen",
					args: { query, limit },
				})
				.then((r) => {
...
mail_merge.serienbrief.mount_vorlagen_browser = ($container, opts = {}) => {
	...
	const loadTemplates = () => {
		...
		return Promise.all([
			frappe.call({
				method: "mail_merge.mail_merge.page.serienbrief_vorlagenbaum.serienbrief_vorlagenbaum.get_vorlagen_for_kategorie",
		...
		if (action === "move") {
			frappe.prompt(
				...
				(values) => {
					...
					frappe
						.call({
							method: "mail_merge.mail_merge.doctype.serienbrief_kategorie.serienbrief_kategorie.move_serienbrief_kategorie",
		...
		if (action === "copy") {
			frappe.prompt(
				...
				(values) => {
					...
					frappe
						.call({
							method: "mail_merge.mail_merge.doctype.serienbrief_kategorie.serienbrief_kategorie.copy_serienbrief_kategorie",
							args: {
								name,
								new_title: newTitle,
...
mail_merge.serienbrief.open_vorlage_picker = (opts = {}) => {
	const dialog = new frappe.ui.Dialog({
	...
	const controller = mail_merge.serienbrief.mount_vorlagen_browser($host, {
		mode: "picker",
