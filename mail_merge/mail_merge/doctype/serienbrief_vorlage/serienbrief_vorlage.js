...
const hv_vorlage_require_placeholder_picker = () =>
	new Promise((resolve) => {
		const assetPath = "/assets/mail_merge/js/serienbrief_placeholder_picker.js";

		const current = window.mail_merge?.serienbrief_placeholder_picker;
		...
		const loadViaBackend = () =>
			frappe
				.call({
					method: "mail_merge.mail_merge.utils.serienbrief_placeholder_picker.get_serienbrief_placeholder_picker_js",
					quiet: true,
				})
				.then((r) => {
		...
		loadViaAsset()
			...
			.finally(() => resolve(window.mail_merge?.serienbrief_placeholder_picker));
	});

const hv_vorlage_require_quill_placeholders = () =>
	new Promise((resolve) => {
		const assetPath = "/assets/mail_merge/js/serienbrief_quill_placeholders.js";

		const current = window.mail_merge?.serienbrief_quill_placeholders;
		...
		const loadViaBackend = () =>
			frappe
				.call({
					method: "mail_merge.mail_merge.utils.serienbrief_quill_placeholders.get_serienbrief_quill_placeholders_js",
					quiet: true,
				})
				.then((r) => {
					const source = r.message || "";
		...
		loadViaAsset()
			.catch(() => loadViaBackend())
			.finally(() => resolve(window.mail_merge?.serienbrief_quill_placeholders));
	});
...
const hv_is_restricted_editor_user = () => {
	return false;
};

const hv_apply_editor_access_mode = (frm) => {
	const restricted = hv_is_restricted_editor_user();
...
const hv_vorlage_doctype_exists = async (doctype) => {
	...
	try {
		const r = await frappe.call({
			method: "mail_merge.mail_merge.utils.serienbrief_placeholder_picker.doctype_exists",
			args: { doctype },
			quiet: true,
		});
...
const hv_vorlage_get_requirements = async (frm) => {
	...
	try {
		const r = await frappe.call({
			method: "mail_merge.mail_merge.doctype.serienbrief_durchlauf.serienbrief_durchlauf.get_template_requirements",
...
const MAIL_MERGE_SPLIT_PREVIEW_STYLE_ID = "mail-merge-split-preview-styles";

const hv_ensure_split_preview_styles = () => {
	if (document.getElementById(MAIL_MERGE_SPLIT_PREVIEW_STYLE_ID)) return;
	const style = document.createElement("style");
	style.id = MAIL_MERGE_SPLIT_PREVIEW_STYLE_ID;
	style.textContent = `
		.hv-split-row {
			display: flex;
			gap: 16px;
...
const hv_build_preview_selector = (frm, host) => {
	...
	const label = $('<span class="hv-preview-selector-label"></span>').text(__("Objekt:"));
	...
	selectBtn.on("click", () => {
		...
		if (!currentDoctype) {
			frappe.msgprint({
				...
				message: __(
					"Setze zuerst das Hauptverteilobjekt auf der Vorlage, damit die Vorschau einen passenden Objekt laden kann."
				),
				indicator: "orange",
			});
			return;
	...
	// nicht mehr gültig — das ausgewählte Objekt passt z.B. nicht zu
...
const hv_update_split_preview = (frm, { immediate } = {}) => {
	...
	const request = frappe
		.call({
			method: "mail_merge.mail_merge.doctype.serienbrief_vorlage.serienbrief_vorlage.render_template_preview_pdf",
			args,
			quiet: true,
		})
		.then((r) => {
...
const hv_get_start_nodes = (frm) => {
	...
	const links = frm._hv_requirements_data?.target_links || [];
	const nodes = [];
...
const hv_open_copy_dialog = (frm) => {
	...
	const dialog = new frappe.ui.Dialog({
		...
		primary_action(values) {
			...
			const request = frappe
				.call({
					method: "mail_merge.mail_merge.doctype.serienbrief_vorlage.serienbrief_vorlage.copy_serienbrief_vorlage",
...
refresh(frm) {
	...
	// normalen Frappe-Berechtigungen.
	frm._hv_show_textbausteine_table = !hv_is_restricted_editor_user();
	...
	frm.add_custom_button(__("In Serienbrief laden"), () => {
		...
		const open_dialog = mail_merge?.serienbrief?.open_new_durchlauf_dialog;
		...
		frappe.require(
			"/assets/mail_merge/js/serienbrief_durchlauf_list.js",
			() => {
				mail_merge.serienbrief.open_new_durchlauf_dialog(prefill);
			}
		);
	});
	if (!frm.is_new()) {
...
haupt_verteil_objekt(frm) {
	...
	// Objekt-Auswahl ungültig (z.B. das Zielobjekt passt nicht zu
