...
class SplitPreviewFrappeDB:
	...
	Wenn eine Vorlage ``frappe.db.get_all("X", filters={"y": objekt.parent})``
	o.ae. macht und ``objekt.parent`` ist ein SplitPreview*-Mock, wirft die
...
def _preview_defaults_for_block(
	...
	if not target_doctype and base_context is not None:
		target_doctype = cstr(getattr(base_context.get("objekt"), "doctype", "") or "").strip()
	path_map = _get_block_default_path_map(block_doc, target_doctype) if target_doctype else {}
...
def _render_segments_via_durchlauf(
	...
	rows = durchlauf._get_iteration_rows()
	if not rows:
		frappe.throw(_("Zielobjekt konnte nicht aus {0} ermittelt werden.").format(iteration_name))
...
def render_template_preview_pdf(
	...
	# und für echte Zielobjekte (iter_dt + iter_name).
...
def render_editor_footer_html(template: str | None = None) -> Dict[str, str]:
	...
	``hv_serienbrief_split_preview``-Flag an, damit App-spezifische Hooks
	Preview-Aufrufe erkennen können.
...
def create_serienbrief_vorlage(
	...
	target_doctype = cstr(haupt_verteil_objekt or "").strip()
...
def get_editor_placeholder_tree(name: str | None = None) -> Dict[str, Any]:
	...
	dt = (getattr(doc, "haupt_verteil_objekt", None) or "").strip() if doc else ""
...
def get_editor_recipients(
	...
	"""Echte Zielobjekte für den Vorschau-Picker."""
	dt = (doctype or "").strip()
...
# Loeschen/Suche/PDF-Vorschau nutzen die bestehenden Editor-Methoden
...
def regenerate_preview_pdf(vorlage_name: str) -> str | None:
	...
	if current_modified and start_modified and current_modified != start_modified:
		try:
			frappe.enqueue(
				"mail_merge.mail_merge.doctype.serienbrief_vorlage.serienbrief_vorlage.regenerate_preview_pdf",
				queue="short",
				timeout=120,
