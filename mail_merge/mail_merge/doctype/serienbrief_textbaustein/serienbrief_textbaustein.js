...
const hv_sync_pdf_field_mappings = (frm, { freeze = true, showAlert = true } = {}) => {
	...
	const request = frappe
		.call({
			method: "mail_merge.mail_merge.doctype.serienbrief_textbaustein.serienbrief_textbaustein.get_pdf_form_fields",
	...
	const dialog = new frappe.ui.Dialog({
		...
		fields: [
			...
			},
			{
				...
				description: __("Z.B. adresse oder positions_liste. Wird im Template als `{{ variable }}` genutzt."),
			},
		],
