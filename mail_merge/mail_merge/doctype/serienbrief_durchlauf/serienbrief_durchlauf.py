...
def _humanize_jinja_error(raw: str) -> str:
	"""Übersetzt häufige Jinja-Fehlerarten in für Nutzer lesbare Hinweise.
	...
	if m:
		...
		return _(
			...
			"Datensatz ist leer. Bitte prüfen, ob alle für diese Vorlage "
			"erforderlichen Datensätze korrekt verknüpft sind."
		).format(field)
...
# Jinja-Rendering aufgelöst werden: ``{{$ objekt.parent.name $}}``.
...
# Der Page-Footer ist kompakt; 16mm lässt Luft für Pfad und kundenspezifische Zeilen.
...
def get_serienbrief_pdf_options() -> Dict[str, str]:
	...
	``mail_merge.install._ensure_serienbrief_dokument_print_format``) — beide
...
class SerienbriefDurchlauf(Document):
	...
	def _create_dokumente(
		...
		iteration_rows = self._get_iteration_rows()
		...
		for idx, row in enumerate(iteration_rows, start=1):
			...
			# Pro Objekt fehlertolerant: ein Render-Fehler markiert NUR diesen
			# Objekt als "Fehler" und bricht den Lauf nicht ab. So bekommt jedes
			...
			try:
				...
				if not segments:
					status = "Übersprungen"
					skip_reason = _("Kein Inhalt für diesen Objekt.")
				else:
					t0 = time.monotonic()
	...
	def _render_dokument_with_print_format(self, doc, format_name: str) -> bytes:
		...
		pdf_bytes = _raw_pdf()
		from mail_merge.mail_merge.utils.pdf_page_numbers import (
	...
	def _apply_template_variables(self, context: Dict[str, Any], template) -> None:
		...
		for variable in variable_defs:
			...
			if resolved is None:
				...
				# strict_variables=False-Pfad (Editor-Live-Preview mit Target),
	...
	def _default_css(self) -> str:
		custom_css = (get_serienbrief_font_face_css() + """
			...
			/* Serienbrief: Letterhead-Layout (Inhalt liefert nur Klassen; Layout kommt aus CSS) */
			.sb-letterhead {
				margin-top: 0.7cm;
			}
			.sb-address-window {
	...
	def _wrap_html(self, body_html: str, paged_polyfill: bool = False, footer_doc=None) -> str:
		...
		paged_script = (
			'<script src="/assets/mail_merge/js/lib/paged.polyfill.js" defer></script>'
			if paged_polyfill
			else ""
		)
...
<html>
	...
	def _get_iteration_rows(self) -> list[Any]:
		rows: list[Any] = []
		for iteration_row in getattr(self, "iteration_objekte", []) or []:
			row = self._build_target_row_from_iteration(iteration_row)
			if row:
				rows.append(row)

		return rows

	def _build_target_row_from_iteration(self, iteration_row):
		...
		row = _IterationTargetRow(row_data)
...
class _LinkResolvingRow:
	...
	def get(self, key: str, default: Any = None) -> Any:
		# ``.get(key)`` wie auf einem Dict, auch für Felder mit Sonderzeichen.
		try:
	...
	def resolve_from_root(root: Any) -> Any:
		...
		while idx < len(segments):
			...
			if isinstance(current, (list, tuple)):
				...
				# wie ``Zielobjekt.vorauszahlung_slots[1]``, wo das Property
			...
			if meta:
				...
				if df:
					if df.fieldtype == "Link" and df.options:
						...
						# Properties wie ``Zielobjekt.kunde`` als Doc, oder
						# Live-Preview-Mock wie ``SplitPreviewObject``),
...
def _mock_context_object(template, iteration_doctype: str | None, context_data: dict[str, Any]):
	return None
...
def _load_value_fields_context(template, iteration_doctype: str | None, iteration_objekt: str | None, context_data: dict[str, Any]):
	...
	if iteration_doctype and iteration_objekt:
		...
		requirements = _collect_template_requirements(template, iteration_doctype)
		rows = serienbrief._get_iteration_rows()
		if rows:
	...
	overrides = _context_overrides(context_data)
	return frappe._dict(
		objekt=_mock_context_object(template, iteration_doctype, context_data),
...
def _get_serienbrief_value_fields_for_doc(
	...
	for key, meta in _collect_auto_variable_field_meta(render_context).items():
		...
		fields.append(
			{
				...
				"source": source,
				"variable": key,
				"variable_type": meta.get("variable_type") or "String",
				"optional": 1,
...
def get_serienbrief_assignments(
	...
	requirements = _collect_template_requirements(template, iteration_doctype)
	iteration_rows = serienbrief._get_iteration_rows()
...
# Objekt/Variablen. E-Mail-Versand = Phase 2.
# ---------------------------------------------------------------------------
...
def start_durchlauf_run(
	...
	frappe.db.commit()
	frappe.enqueue(
		"mail_merge.mail_merge.doctype.serienbrief_durchlauf.serienbrief_durchlauf._run_durchlauf_job",
		queue="long",
		timeout=3600,
		docname=docname,
...
def get_durchlauf_data(docname: str) -> Dict[str, Any]:
	"""Header + Variablen + Objektliste (iteration_objekte ⋈ erzeugte Dokumente)."""
	doc = frappe.get_doc("Serienbrief Durchlauf", docname)
	if not frappe.has_permission("Serienbrief Durchlauf", "read", doc):
		raise frappe.PermissionError
...
def set_run_variables(
	...
	"""Globale Variablenwerte + Pro-Objekt-Overrides speichern (Format passend zu
...
def get_available_recipients(docname: str, query: str | None = None, limit: int = 25) -> Dict[str, Any]:
	"""Iterations-Objekte, die noch nicht im Durchlauf sind (für „Objekt hinzufügen")."""
	doc = frappe.get_doc("Serienbrief Durchlauf", docname)
	if not frappe.has_permission("Serienbrief Durchlauf", "read", doc):
		raise frappe.PermissionError
	dt = cstr(doc.iteration_doctype or "").strip() or cstr(
