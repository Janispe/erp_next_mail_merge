...
frappe
		.call({
			method: "mail_merge.mail_merge.doctype.serienbrief_durchlauf.serienbrief_durchlauf.render_preview",
			args: { doc: frm.doc },
			quiet: true,
		})
		.then((r) => {
...
const _hv_run_pdf_generation = (frm, printFormat, druckSchwarzWeiss = false) => {
	frappe.call({
		method: "mail_merge.mail_merge.doctype.serienbrief_durchlauf.serienbrief_durchlauf.generate_pdf",
		args: {
...
const trigger_serienbrief_pdf = (frm) => {
	...
	// Setting aus Serienbrief Einstellungen lesen (siehe extend_bootinfo)
	...
	const dialog = new frappe.ui.Dialog({
		...
		fields: [
			...
			},
			{
				...
				description: __("Verwendet drucksparende Varianten im Letterhead, sofern die Vorlage diese Option unterstützt."),
			},
		],
...
const hv_open_assignments_overview = async (frm) => {
	...
	try {
		const r = await frappe.call({
			method: "mail_merge.mail_merge.doctype.serienbrief_durchlauf.serienbrief_durchlauf.get_serienbrief_assignments",
			args: { doc: frm.doc },
			quiet: true,
		});
		data = r.message || {};
	...
	const sections = rows.map((row) => {
		const iterationLabel = row?.iteration?.doctype && row?.iteration?.name
			...
			: "";
		const header = `${__("Objekt")} ${row.index || ""}`.trim();
...
const trigger_serienbrief_html = (frm) => {
	...
	frappe.call({
		method: "mail_merge.mail_merge.doctype.serienbrief_durchlauf.serienbrief_durchlauf.generate_html",
...
const hv_get_iteration_setters = () => {
	return {};
};
...
const hv_open_iteration_picker = (frm) => {
	...
	// Picker stretchen — Breite kommt aus Serienbrief Einstellungen.
	if (window.mail_merge?.ui?.widen_modal) {
		window.mail_merge.ui.widen_modal(dialog.dialog || dialog);
	}
};
...
const hv_mount_durchlauf_viewer = (frm) => {
	...
	if ($wrapper.find("iframe.mail-merge-durchlauf-frame").length) return;

	const src = `/assets/mail_merge/serienbrief_durchlauf/index.html?docname=${encodeURIComponent(
		frm.doc.name
	)}&v=${Date.now()}`;
	$wrapper.empty();
	const $frame = $(
		`<iframe class="mail-merge-durchlauf-frame" src="${src}" title="Serienbrief Durchlauf"></iframe>`
	...
	const onMessage = (event) => {
		...
		if (!msg || msg.source !== "mail-merge" || msg.type !== "rpc") return;
		...
		if (!frm.is_new()) {
			...
			if (cint(frm.doc.docstatus) === 0) {
				frm.add_custom_button(__("Dokumente neu erzeugen"), async () => {
					...
					if (!confirmed) return;
					await frappe.call({
						method: "mail_merge.mail_merge.doctype.serienbrief_durchlauf.serienbrief_durchlauf.regenerate_dokumente",
			...
			if (frm.doc.status === "Läuft") {
				frm.add_custom_button(__("Lauf als fehlgeschlagen markieren"), async () => {
					...
					if (!confirmed) return;
					await frappe.xcall(
						"mail_merge.mail_merge.doctype.serienbrief_durchlauf.serienbrief_durchlauf.mark_durchlauf_failed",
						{ docname: frm.doc.name }
					);
