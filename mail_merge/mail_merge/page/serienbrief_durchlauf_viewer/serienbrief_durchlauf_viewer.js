...
//   apps/mail_merge/mail_merge/public/serienbrief_durchlauf/
// und wird unter /assets/mail_merge/serienbrief_durchlauf/ ausgeliefert.
//
...
function dl_build_src(docname, vorlage) {
	let s = `/assets/mail_merge/serienbrief_durchlauf/index.html?v=${Date.now()}`;
...
// Verlassen alle postMessage-Aktionen (Lauf starten, Zielobjekte, PDF …) tot.
...
frappe.pages["serienbrief_durchlauf_viewer"].on_page_load = function (wrapper) {
	...
	const $frame = $(
		`<iframe class="mail-merge-durchlauf-frame" src="${dl_build_src(_currentDocname, dl_consume_vorlage())}" title="Serienbrief Durchlauf"></iframe>`
